...
export default function WhyConsultAmerica() {
  return (
    <Section id="why-consultamerica" className="bg-white text-[#05070d]">
      <Container>
        <div className="grid gap-12 lg:grid-cols-12 lg:gap-16">
          <div className="lg:col-span-4">
            <span className="ca-eyebrow text-black/45">WHY CONSULTAMERICA</span>
          </div>
          ...
          >
            ...
            <p className="mt-8 max-w-3xl text-lg leading-8 text-black/65">
              We combine enterprise-scale expertise with senior involvement,
              practical execution, and flexible delivery—giving clients the
              depth required for complex transformation without unnecessary
              layers.
            </p>
          </motion.div>
        </div>

        <div className="mt-20 grid border-t border-black/10 md:grid-cols-2 lg:grid-cols-3">
          {differentiators.map((item, index) => (
            <motion.article
              ...
              className="group relative border-b border-black/10 p-8 md:min-h-[300px] md:border-r md:p-10 md:[&:nth-child(2n)]:border-r-0 lg:min-h-[320px] lg:[&:nth-child(2n)]:border-r lg:[&:nth-child(3n)]:border-r-0"
            >
              <div
                aria-hidden="true"
              ...
              <span className="ca-eyebrow text-black/40">{item.number}</span>

              <h3 className="mt-12 text-2xl font-medium tracking-[-0.035em] text-[#05070d] transition-colors duration-300 group-hover:text-[var(--ca-blue)]">
                {item.title}
              </h3>

              <p className="mt-5 max-w-sm text-base leading-7 text-black/55">
                {item.description}
              </p>
            </motion.article>
          ))}
        ...
        >
          <p className="max-w-5xl text-4xl font-medium leading-[1.08] tracking-[-0.045em] text-[#05070d] md:text-6xl">
